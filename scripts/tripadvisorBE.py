...
# This is the framework for chaining the Extractors required to do the tripadvisorBE extraction
...
Extractor1 = "72287b01-8793-4d83-981d-354526a16855" #Replace this with the GUID of YOUR Extractors (Locations List)
...
with open('TripAdvisorBE_step1_urls.csv','wb') as f: # if you want to upload URLs directly to the import.io dashboard you can write the results to a .csv file and upload it manually
    writer = csv.writer(f)

    # 1. Get all links generated by Extractor1
    page = 0
    total = 960 # there are 113 pages with step 20
    page_counter = 1
    error_page = 0
    while page < total:
        ...
        try:
            page = str(page)
            jsonurl = "https://extraction.import.io/query/extractor/"+Extractor1+"?_apikey="+apikey+"&url=https%3A%2F%2Fwww.tripadvisor.com%2FRestaurants-g188634-oa"+page+"-Belgium.html%23LOCATION_LIST"
...
#now you can open TripAdvisorBE_step1_urls.csv with generated urls and use all these links for the next extractor in dash - JustEat 2 - TripAdvisorBE (restList) 8be5a132-092b-423d-95f8-ea6d5484096c 
